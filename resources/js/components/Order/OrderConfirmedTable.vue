  ...
  <VTable height="250" fixed-header>
    ...
    <tbody>
      <tr v-for="item in confirmedOrders" :key="item.id">
        ...
        <td class="text-center">
          <div class="status-dot" :class="{ 'status-dot-read': item.status === 'Rejected' }">
            <VChip v-if="item.status == 'Approved'" color="success" size="x-small" >{{ item.status }}</VChip>
            <VChip v-else color="error" size="x-small">{{ item.status }}</VChip>
          </div>
        </td>

        <td class="text-center">
          <VBtn color="primary" size="small" @click="viewApprovedOrder(item)">View</VBtn>
        </td>
      </tr>
    </tbody>
  </VTable>
